...
#'
#' @description
#' Analyzes a YAML metadata file to extract all domain and variable references
#' from column sources and origins. This function helps track dependencies
#' between different domains and variables in the metadata structure.
#'
#' @param path Character string specifying the file path to the YAML metadata
#' file.
#'
...
#'
#' References are expected to be in the format "DOMAIN.VARIABLE"
#' (e.g., "ADSL.USUBJID"). The function handles both simple references and
#' complex patterns within origin descriptions.
#'
...
#'
#' @note
#' The function will generate a warning if it encounters incomplete source or
#' origin references (missing domain or variable components).
#'
#' @examples
#' \dontrun{
...
extract_references_from_yaml <- function(path) {
  ...
  # This looks for uppercase letters/numbers followed by a period and more
  ...
  direct_columns <- column_data |>
    dplyr::mutate(source2 =
                    dplyr::case_when(!is.na(.data$source) ~ source,
                                     grepl(pattern, column) ~ column)) |>
    dplyr::filter(!is.na(.data$source2)) |>
    dplyr::mutate(source2 = stringr::str_split(.data$source2, "\\.")) |>
    tidyr::unnest_wider(.data$source2, names_sep = "_") |>
    dplyr::rename(domain = .data$source2_1, variable = .data$source2_2)
  ...
  origin_columns <- column_data |>
    ...
    dplyr::mutate(origin_source =
                    stringr::str_extract_all(.data$origin, pattern)) |>
    tidyr::unnest_longer(.data$origin_source) |>
    # If it is an extended reference, e.g. SDTM.FA.FALNKID, only keep the last
    # bit
    # Pull out the domain and variable in seperate columns
    dplyr::mutate(
      origin_source =
        stringr::str_extract(.data$origin_source,
                             "[A-Z][A-Z0-9]+\\.[A-Z][A-Z0-9]+$"),
      origin_source =
        stringr::str_split(.data$origin_source, "\\.")
    ) |>
    tidyr::unnest_wider(.data$origin_source, names_sep = "_") |>
    dplyr::rename(domain = .data$origin_source_1,
                  variable = .data$origin_source_2) |>
    # Remove any left-over dataset references
    dplyr::filter(!.data$domain %in% c("SDTM", "ADM", "ADAM", "METADATA"))
  ...
  incomplete_source <- all_columns |>
    dplyr::filter(is.na(.data$domain) | is.na(.data$variable)) |>
  ...
  domain_variable <- all_columns |>
    ...
    dplyr::distinct(.data$domain, .data$variable) |>
    # Transform dataframe into a list
    dplyr::group_by(.data$domain) |>
    dplyr::summarise(variable = list(.data$variable))
