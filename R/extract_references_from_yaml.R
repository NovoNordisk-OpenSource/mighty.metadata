...
#' column sources and methods. This function helps track dependencies between
...
#' 2. References embedded in method descriptions
#' 3. References in value-level metadata
#'
#' References are expected to be in the format "DOMAIN.VARIABLE" (e.g., "ADSL.USUBJID").
#' The function handles both simple references and complex patterns within method
#' descriptions.
#'
...
#'
#' @note
#' The function will generate a warning if it encounters incomplete source or method
...
extract_references_from_yaml <- function(path) {
  ...
  # Extract domain-value pairs from references in column or value method
  method_colums <- column_data |>
    bind_rows(value_data) |>
    dplyr::filter(!is.na(method)) |>
    # Extract everything in method that looks like a domain.variable reference
    dplyr::mutate(method_source = stringr::str_extract_all(method, pattern)) |>
    tidyr::unnest_longer(method_source) |>
    ...
    dplyr::mutate(method_source = stringr::str_extract(method_source, "[A-Z][A-Z0-9]+\\.[A-Z][A-Z0-9]+$"),
                  method_source = stringr::str_split(method_source, "\\.")) |>
    tidyr::unnest_wider(method_source, names_sep = "_") |>
    dplyr::rename(domain = method_source_1, variable = method_source_2) |>
    # Remove any left-over dataset references
    dplyr::filter(!domain %in% c("SDTM", "ADM", "ADAM", "METADATA"))

  all_columns <- direct_columns |>
    dplyr::bind_rows(method_colums)
  ...
  if (incomplete_source != "") {
    warning("Warning in table ", table_name,
            ": Incomplete source/method for variables: ", incomplete_source)
  }
