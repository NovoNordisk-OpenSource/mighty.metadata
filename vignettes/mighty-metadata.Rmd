...
are stored in `@study` as a `study_config()` object and mighty framework
configuration from `_mighty.yml` is stored in `@mighty` as a `mighty_config()`
object. Both are `NULL` when the corresponding file is absent. The `@` operator
accesses properties of S7 objects:
...
`include` can be applied to columns, rows, parameters, and individual columns within a parameter.

## Creating Flat Metadata Tables

A set of `create_md_*()` functions flattens the study specifications into
tibbles. This is the format consumed by downstream mighty tools.

`create_md_table()` gives one row per dataset:

```{r}
create_md_table(study)
```

`create_md_col()` gives one row per column:

```{r}
create_md_col(study)
```

`create_md_param()` gives one row per BDS parameter:

```{r}
create_md_param(study)
```

`create_md_values()` gives one row per column defined inside a parameter,
i.e. the value level metadata:
...
for conditional specifications, and the `create_md_*()` functions for flat
output.
